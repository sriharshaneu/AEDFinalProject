...
public class OrderForm extends javax.swing.JFrame {
	...
	static boolean isOrderTaken;
	public static boolean freezeOrderBtn;
	public static boolean orderReady = false;
	...
	public OrderForm(Customer customer, String Ids) {
		...
		boolean isLastDelivered = checkCustomerLastOrders(customer);
		if (isLastDelivered) {
	...
	OrderForm(Waitor waitor, String ordersString) {
		initComponents();
		this.waitor = waitor;
		menuIds = ordersString;
		String iiid = String.valueOf(waitor.getId());
		boolean isLastDelivered = checkWaitorLastOrders(waitor);
		if (isLastDelivered) {
	...
	private boolean checkCustomerLastOrders(Customer c) {
...
//			}
	...
	private boolean checkWaitorLastOrders(Waitor w) {
		Order tempOrder = new Order();
...
//            }
	...
	public boolean checkStatusInChefOrders(int order_id) {
		PreparedStatement ps;
		ResultSet rs;
		final String QUERY = "SELECT * FROM chef_orders WHERE order_id = (?);";
		try {
			...
			Logger.getLogger(BillForm.class.getName()).log(Level.SEVERE, null, e);
			e.printStackTrace();
		}
		return false;
	}
    ...
    private void initComponents() {
        ...
        cbx_promoCode.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                cbx_promoCodeActionPerformed(evt);
            }
        });
        ...
        cbx_choose_order.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                cbx_choose_orderActionPerformed(evt);
            }
        });
    ...
    private void btn_showBillActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_showBillActionPerformed
	    ...
	    try {
		    ...
		    if (cbx_choose_order.getSelectedItem() == null || cbx_choose_order.getSelectedIndex() == -1) {
			    //notFirstVisit = false;
			    JOptionPane.showConfirmDialog(this, "Please select atleast one item.");
		    } else {
			    ...
			    JOptionPane.showMessageDialog(this, "Order being prepared. Chef will dispatch your order", "Ticking..", JOptionPane.INFORMATION_MESSAGE);

			    setInitialValues();
			    System.out.println("the curent list is :" + currentOrder.orderList);
    ...
    private void btn_orderStatusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_orderStatusActionPerformed
	    ...
	    try {
		    ...
		    System.out.println("waitor is ======> " + waitor.getId());

		    String selected = (String) cbx_choose_order.getSelectedItem();

		    //if ( selected == null) {
		    //    JOptionPane.showMessageDialog(this, "You haven't placed any order", "Warning", JOptionPane.WARNING_MESSAGE);
		    //} else {
		    ...
		    if (customer.getOrders().isEmpty()) {
			    ...
			    if (/*currentOrder.getStatus()*/customer.getTempOrder().getStatus()) //or currentOrder.getStatus()
			    {
				    ...
				    System.out.println("Current customer" + customer.getId());
				    System.out.println("Current waitor" + waitor.getId());

				    if (customer.getId() > 0) {
					    System.out.println("current order:" + currentOrder + "   bill:" + bill);
					    if (orderReady) {
				    ...
				    } else if (waitor.getId() > 0) {

					    System.out.println(" else if (waitor.getId() > 0) line 749 ===> OrderForm.java");
					    if (orderReady) {
						    JOptionPane.showMessageDialog(this, "Your order has been delivered");
					    } else {
						    JOptionPane.showMessageDialog(this, "Still being cooked");
    ...
    private void btnBestSellersCustomerActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnBestSellersCustomerActionPerformed
        ...
        private void cbx_promoCodeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cbx_promoCodeActionPerformed
		// TODO add your handling code here:
        }//GEN-LAST:event_cbx_promoCodeActionPerformed

        private void cbx_choose_orderActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cbx_choose_orderActionPerformed
		// TODO add your handling code here:
        }//GEN-LAST:event_cbx_choose_orderActionPerformed
